...
interpPacket :: Packet -> Int
interpPacket (Literal _ _ n) = n
interpPacket (Operator _ 0 subps) = interpAux sum subps
interpPacket (Operator _ 1 subps) = interpAux product subps
interpPacket (Operator _ 2 subps) = interpAux (foldr1 min) subps
interpPacket (Operator _ 3 subps) = interpAux (foldr1 max) subps
interpPacket (Operator _ 5 subps) = interpAux (\(f:t:[]) -> if f > t then 1 else 0) subps
interpPacket (Operator _ 6 subps) = interpAux (\(f:t:[]) -> if f < t then 1 else 0) subps
interpPacket (Operator _ 7 subps) = interpAux (\(f:t:[]) -> if f == t then 1 else 0) subps

interpAux :: ([Int] -> Int) -> [Packet] -> Int
interpAux func = func . map interpPacket

solve :: String -> String
solve inp = let (Right p) = runParser packet . hex2bin $ inp
         in show . interpPacket . result $ p
