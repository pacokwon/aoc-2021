...
main :: IO ()
main = interact $ showFoldedPos . (uncurry afterFold) . parse . lines
...
afterFold :: [Fold] -> [Pos] -> [Pos]
afterFold (f:fs) poss = afterFold fs (map (foldAlong f) poss)
afterFold []     poss = nub poss
